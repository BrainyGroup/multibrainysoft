...
watch(form, function (newForm) {
  ...
  timerId = setTimeout(() => {
    ...
    router.get('/organizations', queryParams, options)
  }, 150)
}, { deep: true })
...
<template>
    ...
    <AuthenticatedLayout  :translations="translations">
        <template #header>
            <h1 class="mb-1 ml-4 text-3xl font-bold">{{ t('organizations') }}</h1>
        </template>
            ...
            <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
                <Head :title="`${t('organizations')}`" />
                           ...
                           <div class="flex items-center justify-between mb-6">
                                ...
                                <Link v-if="can.create_organization" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition" href="/organizations/create">
                                    <span>{{ t('create') }}</span>
                            ...
                            <table class="w-full whitespace-nowrap">
                                ...
                                <tr class="text-left font-bold">
                                    ...
                                    <th class="pb-2 pt-2 px-6">{{ t('account number') }}</th>
                                ...
                                <tr v-for="organization in organizations.data" :key="organization.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                                    <td class="border-t">
                                        <Link class="flex items-center px-6 py-2 focus:text-indigo-500 text-sm" :href="`/organizations/${organization.id}/edit`">
                                            {{ organization.name }}
                                    ...
                                    <td class="border-t">
                                        <Link class="flex items-center px-6 py-2 focus:text-indigo-500 text-sm" :href="`/organizations/${organization.id}/edit`">
                                            {{ organization.statutory_type_name }}
                                            <icon v-if="organization.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400" />
                                        </Link>
                                    </td>
                                    ...
                                    <td class="border-t">
                                        <delete-button  @delete="deleteOrganization(`${organization.id}`)">Delete</delete-button>                      
                                    </td>
