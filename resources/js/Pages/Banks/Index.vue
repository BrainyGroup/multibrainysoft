...
watch(form, function (newForm) {
  ...
  timerId = setTimeout(() => {
    ...
    router.get('/banks', queryParams, options)
  }, 150)
}, { deep: true })
...
<template>
    ...
    <AuthenticatedLayout :translations="translations">
        <template #header>
            <h1 class="mb-1 ml-4 text-3xl font-bold">{{ t('banks') }}</h1>
        </template>
                           ...
                           <div class="flex items-center justify-between mb-6">
                            ...
                            <table class="w-full whitespace-nowrap">
                                ...
                                <tr v-for="bank in banks.data" :key="bank.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                                    <td class="border-t">
                                        <Link class="flex items-center px-6 py-2 focus:text-indigo-500 text-sm" :href="`/banks/${bank.id}/edit`">
                                            {{ bank.name }}
                                            <icon v-if="bank.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400" />
                                        </Link>
                                    </td>
                                    ...
                                    <td class="border-t">
                                        <delete-button  @delete="deleteBank(`${bank.id}`)">{{ t('delete') }}</delete-button>                      
                                    </td>


                                </tr>
                                <tr v-if="banks.length === 0">
                                    <td class="px-6 py-4 border-t" colspan="4">No banks found.</td>
                                </tr>
                                </tbody>
                            </table>
                            </div>
