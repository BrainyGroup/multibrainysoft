...
watch(form, function (newForm) {
  ...
  timerId = setTimeout(() => {
    ...
    router.get('/companies', queryParams, options)
  }, 150)
}, { deep: true })
...
<template>
    ...
    <AuthenticatedLayout  :translations="translations ">
        <template #header>
            <h1 class="mb-1 ml-4 text-3xl font-bold">{{ t('companies') }}</h1>
        </template>
            ...
            <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">  
                <Head :title="`${t('companies')}`" />                        
                           ...
                           <div class="flex items-center justify-between mb-6">
                                ...
                                <Link v-if="can.create_company" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition" href="/companies/create">
                                    <span>{{ t('create') }}</span>
                                    <span class="hidden md:inline">&nbsp;{{ t('company') }}</span>
                                </Link>
                            </div>
                            ...
                            <table class="w-full whitespace-nowrap">
                                ...
                                <tr class="text-left font-bold">
                                    ...
                                    <th class="pb-2 pt-2 px-6">{{ t('tenant id') }}</th>
                                    <th class="pb-2 pt-2 px-6">{{ t('country id') }}</th>
                                ...
                                <tr v-for="company in companies.data" :key="company.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                                    ...
                                    <td class="border-t">
                                        <Link class="flex items-center px-6 py-2 focus:text-indigo-500 text-sm" :href="`/companies/${company.id}/edit`">
                                            {{ company.id }}
                                    ...
                                    <td class="border-t">
                                        <delete-button  @delete="deleteCompany(`${company.id}`)">Delete</delete-button>                      
                                    </td>
