...
const deleteAllowance = (id) => {
    router.delete(route("allowances.destroy", id));   
...
watch(form, function (newForm) {
  ...
  timerId = setTimeout(() => {
    ...
    router.get('/allowances', queryParams, options)
  }, 150)
}, { deep: true })
...
<template>
    ...
    <AuthenticatedLayout  :translations="translations">
        <template #header>
            <h1 class="mb-1 ml-4 text-3xl font-bold">{{ t('allowances') }}</h1>
        </template>

        <div class="mx-4">
            <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8"> 
                <Head :title="`${t('allowances')}`" />                         
                           ...
                           <div class="flex items-center justify-between mb-6">
                            ...
                            <table class="w-full whitespace-nowrap">
                                ...
                                <tr class="text-left font-bold">
                                    ...
                                    <th class="pb-2 pt-2 px-6">{{ t('allowance type') }}</th> 
                                ...
                                <tr v-for="(allowance, index) in allowances.data" :key="allowance.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                                    <td class="border-t">
                                        <div class="flex items-center px-6 py-2 focus:text-indigo-500 text-sm" >
                                            {{ index + 1 }}
                                            <icon v-if="allowance.deleted_at" name="trash" class="flex-shrink-0 ml-2 w-3 h-3 fill-gray-400" />
                                        </div>
                                    </td>
                                    ...
                                    <td class="border-t">
                                        <delete-button  @delete="deleteAllowance(`${allowance.id}`)">Delete</delete-button>                      
                                    </td>


                                </tr>
                                <tr v-if="allowances.length === 0">
                                    <td class="px-6 py-4 border-t" colspan="4">No allowance found.</td>
                                </tr>
                                </tbody>
                            </table>
                            </div>
                            <pagination class="mt-6" :links="allowances.links" /> 
                        </div>
                    <SectionBorder />
                </div>
    </AuthenticatedLayout>
